...
my $epochLastdate =
  timelocal( 59, 59, 23, $mday, int($mon) - 1, $year - 1900 ) + $ONEDAY;

do {
    my (
        $seconds, $minutes, $hours, $day_of_month, $month,
        $year,    $wday,    $yday,  $isdst
    ) = localtime($epochLastdate);
    $year += 1900;
    $month++;
    $month        = stuff($month);
    ...
    print
"http://www.forexite.com/free_forex_quotes/$year/$month/$day_of_month$month$shortyear.zip\n";
